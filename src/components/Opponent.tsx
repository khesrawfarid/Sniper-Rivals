...
export const UploadedCharacter = ({
  playerState,
  outfitColor = "#2b6cb0",
  eyeColor = "#1a202c",
  position = [0, -0.85, 0],
  ...
  useFrame((state, delta) => {
    ...
    if (dist > 5) {
      ...
      meshRef.current.position.lerp(
        targetPos.current,
        THREE.MathUtils.clamp(12 * delta, 0, 1),
      );
    }
    ...
    meshRef.current.rotation.y +=
      diff * THREE.MathUtils.clamp(15 * delta, 0, 1);

    // Bot Animation
    if (botGroupRef.current) {
      const timeOffset = parseInt(id.replace(/\D/g, "")) || 0;
      botGroupRef.current.position.y =
        0.4 + Math.sin(state.clock.elapsedTime * 2 + timeOffset) * 0.05;
    }
  });
